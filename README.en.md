<p align="center">
  ...
  <br>
  <a href="#for-ai-agents"><img src="https://img.shields.io/badge/MCP_Server-50_tools-blueviolet.svg" alt="MCP: 50 tools"></a>
...
Every command speaks JSON. The built-in MCP server gives AI agents 50 tools to manage your fork autonomously -- from init through conflict resolution. No human in the loop required.
...
- :electric_plug: **MCP server (50 tools)** -- full fork management from init through conflict resolution.
...
bingo-light was designed from day one for AI agents. Every command speaks JSON. The MCP server exposes 50 tools covering the full lifecycle from `init` to `conflict-resolve`. Non-interactive mode is the default when stdin is not a TTY.
...
| AI / MCP integration | **50 tools** | No | No | No | No |
...
mcp-server.py        MCP server (zero-dep Python 3, 50 tools, JSON-RPC 2.0)
