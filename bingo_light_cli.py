...
def _format_history(result: dict) -> str:
    """Format history/log result."""
    entries = result.get("history", result.get("entries", []))
    if not entries:
        return "No sync history."
    lines = []
    for e in entries:
        ts = e.get("timestamp", "")
        msg = e.get("message", e.get("summary", ""))
        lines.append(f"  {_c(DIM, ts)}  {msg}")
    return "\n".join(lines)
...
def _format_workspace(result: dict) -> str:
    ...
    if repos is not None:
        ...
        for r in repos:
            alias = r.get("alias", "?")
            path = r.get("path", "?")
            lines.append(f"  {_c(CYAN, alias)}  {_c(DIM, path)}")
        return "\n".join(lines)
    # workspace sync
...
def _format_init(result: dict) -> str:
    ...
    return (
        f"{_c(GREEN, 'OK')} Fork tracking initialized.\n"
        f"  Upstream: {upstream}\n"
        f"  Branch: {branch}\n"
...
def show_help() -> str:
  ...
  {b}Multi-repo:{r}
    {cy}workspace{r} init|add|status|sync|list  Multi-repo management
...
def _dispatch_workspace(args: argparse.Namespace, repo: Repo) -> dict:
    ...
    if not wcmd:
        raise BingoError(
            "workspace requires a subcommand: init, add, list, sync, status"
        )
    ...
    if wcmd == "status":
        # workspace status falls back to workspace list
        return repo.workspace_list()
